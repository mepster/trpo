...
def broadcast_policy_scaler_val(policy, scaler, val_func):
    rank0_bcast_wts(policy.sess, policy.g, 'policy')
    #mpi_util.chksum(policy.sess, policy.g, 'policy')
    rank0_bcast_wts(val_func.sess, val_func.g, 'val_func')
    #mpi_util.chksum(val_func.sess, val_func.g, 'val_func')
    MPI.COMM_WORLD.Bcast(scaler.vars, root=0)
    MPI.COMM_WORLD.Bcast(scaler.means, root=0)
    scaler.m = MPI.COMM_WORLD.bcast(scaler.m, root=0)
    scaler.n = MPI.COMM_WORLD.bcast(scaler.n, root=0)
    # print(scaler.vars)
    # print(scaler.means)
    # print(scaler.m)
    # print(scaler.n)
    return policy, scaler, val_func

def gather_trajectories(trajectories):
    return MPI.COMM_WORLD.gather(trajectories, root=0)
...
def steps_sec(stepcnt):
    ...
    # print0('cumsum_steps=%d' % cumsum_steps)   # only for step counting
    etime = time.time()
    # print0('%d_steps_sec=%d' % (rank, stepcnt/(etime-tstart)))
    tstart = etime
